...
order newOrder = d.createOrderWithFood(note, 1, 1, 0, 1, foodID, quantity);
            if (newOrder != null) {
    response.sendRedirect("menu?message=Failed to create order. Please try again.");
} else {
    response.sendRedirect("menu?message=Order created successfully!");
}
