...
"    WHERE CustomerID = ?);";
   ...
   public static void main(String[] args) {
    ...
    try {
        ...
        List<checkoutTableOrder> orders = d.getstaffTableOrder(2);

        // Kiểm tra và in ra danh sách các đơn hàng
        if (orders.isEmpty()) {
            System.out.println("No orders found.");
        } else {
            System.out.println("List of Orders:");
            for (checkoutTableOrder order : orders) {
                // In thông tin đơn hàng
                System.out.println(
                                   ", Table Name: " + order.getTableName() +
                                   ", Customer Name: " + order.getCustomerName() +
                                   ", Guest Name: " + order.getGuestName() );
